<script setup>
import { ref, computed, watch } from 'vue'
import { router } from '@inertiajs/vue3'
import moment from 'moment'

const props = defineProps({
    open:    Boolean,
    teacher: Object,
    day:     Object,
    mode:    { type: String, default: 'book' },
})

const emit = defineEmits(['close'])

const slots            = ref([])
const loading          = ref(false)
const selectedSlotTime = ref('')
const error            = ref('')
const submitting       = ref(false)

const selectedSlot = computed(() => slots.value.find(s => s.start_time === selectedSlotTime.value) ?? null)

const fetchSlots = async () => {
    if (!props.teacher || !props.day) return

    loading.value          = true
    slots.value            = []
    selectedSlotTime.value = ''
    error.value            = ''

    try {
        const url = route('parent.slots.index', { teacher: props.teacher.id, date: moment.utc(props.day.date).unix() })
        const res = await fetch(url)
        slots.value = await res.json()
    } finally {
        loading.value = false
    }
}

watch(() => props.open, (val) => {
    if (val && props.mode === 'book') fetchSlots()
    if (!val) error.value = ''
})

const submit = () => {
    if (!selectedSlot.value) return

    submitting.value = true
    error.value      = ''

    router.post(
        route('parent.slots.store', { teacher: props.teacher.id, date: moment.utc(props.day.date).unix() }),
        { start_time: selectedSlot.value.start_time },
        {
            onSuccess: () => {
                submitting.value = false
                emit('close')
            },
            onError: (errors) => {
                submitting.value = false
                error.value = errors.booking ?? 'Something went wrong.'
            },
        }
    )
}

const cancel = () => {
    submitting.value = true
    error.value      = ''

    router.delete(
        route('parent.slots.destroy', { teacher: props.teacher.id, date: moment.utc(props.day.date).unix() }),
        {
            onSuccess: () => {
                submitting.value = false
                emit('close')
            },
            onError: () => {
                submitting.value = false
                error.value = 'Could not cancel the booking.'
            },
        }
    )
}

const formatTime = (t) => t?.slice(0, 5) ?? t

const availableSlots = computed(() => slots.value.filter(s => s.is_available))

const labelClass = 'text-sm font-medium text-gray-700'
const selectClass = 'w-full mt-1 border border-gray-200 rounded-xl px-3 py-2 text-sm bg-gray-50 focus:bg-white focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition'
</script>

<template>
    <Transition
        enter-active-class="transition ease-out duration-200"
        enter-from-class="opacity-0"
        enter-to-class="opacity-100"
        leave-active-class="transition ease-in duration-150"
        leave-from-class="opacity-100"
        leave-to-class="opacity-0"
    >
        <div v-if="open" class="fixed inset-0 flex items-center justify-center z-50">
            <div class="absolute inset-0 bg-black/40" @click="$emit('close')" />

            <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg p-8 z-10 border border-gray-100">

                <div class="flex items-start justify-between mb-6">
                    <div>
                        <h3 class="text-lg font-semibold text-gray-900">
                            {{ mode === 'cancel' ? 'Cancel booking' : 'Book appointment' }}
                        </h3>
                        <p class="text-sm text-gray-500 mt-0.5">
                            {{ teacher?.name }} &mdash; {{ day?.label }}
                        </p>
                    </div>
                    <button @click="$emit('close')" class="text-gray-400 hover:text-gray-600 text-xl leading-none">&times;</button>
                </div>

                <!-- Error -->
                <div v-if="error" class="mb-4 px-4 py-3 bg-red-50 border border-red-200 rounded-xl text-sm text-red-700">
                    {{ error }}
                </div>

                <!-- Cancel confirmation -->
                <div v-if="mode === 'cancel'" class="mb-8">
                    <p class="text-sm text-gray-600">
                        Are you sure you want to cancel your booking with
                        <span class="font-semibold text-gray-900">{{ teacher?.name }}</span>
                        on <span class="font-semibold text-gray-900">{{ day?.label }}</span>?
                    </p>
                </div>

                <!-- Slots (book mode) -->
                <div v-else class="mb-8">
                    <label :class="labelClass">Select a time slot</label>

                    <div v-if="loading" class="mt-2 text-sm text-gray-400">Loading slots…</div>

                    <div v-else-if="availableSlots.length === 0" class="mt-2 text-sm text-gray-400">
                        No slots available for this day.
                    </div>

                    <select v-else v-model="selectedSlotTime" :class="selectClass">
                        <option value="">— choose a time —</option>
                        <option
                            v-for="slot in availableSlots"
                            :key="slot.start_time"
                            :value="slot.start_time"
                        >
                            {{ formatTime(slot.start_time) }} – {{ formatTime(slot.end_time) }}
                        </option>
                    </select>
                </div>

                <!-- Actions -->
                <div class="flex justify-end gap-3">
                    <button
                        @click="$emit('close')"
                        class="px-4 py-2 text-sm text-gray-600 hover:text-gray-800"
                    >
                        Close
                    </button>
                    <button
                        v-if="mode === 'cancel'"
                        @click="cancel"
                        :disabled="submitting"
                        class="px-5 py-2 bg-red-600 text-white rounded-xl shadow hover:bg-red-700 transition disabled:opacity-40 disabled:cursor-not-allowed"
                    >
                        {{ submitting ? 'Cancelling…' : 'Cancel booking' }}
                    </button>
                    <button
                        v-else
                        @click="submit"
                        :disabled="!selectedSlot || submitting"
                        class="px-5 py-2 bg-indigo-600 text-white rounded-xl shadow hover:bg-indigo-700 transition disabled:opacity-40 disabled:cursor-not-allowed"
                    >
                        {{ submitting ? 'Booking…' : 'Book slot' }}
                    </button>
                </div>

            </div>
        </div>
    </Transition>
</template>
